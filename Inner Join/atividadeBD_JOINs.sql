...
SELECT MAX (numPaginas) AS 'Livro com maior número de páginas' FROM tbLivro

--2)	O menor número de páginas entre todos os Livros
SELECT MIN (numPaginas) AS 'Livro com menor número de páginas' FROM tbLivro

--3)	A média de página entre todos os livros
SELECT AVG (numPaginas) AS 'Média de página entre todos os livros' FROM tbLivro

--4)	A soma do número de páginas dos livros de editora de código 01
SELECT SUM (numPaginas) AS 'Soma das páginas dos livros de editora de código 2' FROM tbLivro WHERE codEditora = 2

--5)	A soma dos números de páginas dos livros que começam com a letra A
SELECT SUM (numPaginas) AS 'Somas das páginas dos livros que começam com A' FROM tbLivro WHERE nomeLivro LIKE 'A%'

--6)	A média dos números de páginas dos livros que sejam do autor de código 03
SELECT AVG (numPaginas) AS 'A média dos números de páginas dos livros que sejam do autor de código 3' FROM tbLivro WHERE codAutor = 3

--7)	A quantidade de livros da editora de código 04 
SELECT COUNT (nomeLivro) AS 'A quantidade de livros da editora de código 4'  FROM tbLivro WHERE codEditora = 4

--8)	A média do número de páginas dos livros que tenham a palavra “ESTRELA“ em seu nome.
SELECT AVG (codLivro) AS 'A média do número de páginas dos livros que tenham a palavra “ESTRELA“ em seu nome.' FROM tbLivro WHERE nomeLivro LIKE '%ESTRELA%'

--9)	A quantidade de livros que tenham a palavra “POEMA” em seu nome.
SELECT COUNT (codLivro) AS 'A quantidade de livros que tenham a palavra “POEMA” em seu nome.' FROM tbLivro WHERE nomeLivro LIKE '%POEMA%'
